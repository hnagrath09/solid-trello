...
var (
	userAllColumns            = []string{"id", "email", "password", "name", "created_at"}
...
// SetBoards removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's Boards accordingly.
// Replaces o.R.Boards with related.
// Sets related.R.User's Boards accordingly.
func (o *User) SetBoards(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Board) error {
	query := "update \"boards\" set \"user_id\" = null where \"user_id\" = $1"
	...
	return o.AddBoards(ctx, exec, insert, related...)
}

// RemoveBoards relationships from objects passed in.
// Removes related items from R.Boards (uses pointer comparison, removal does not keep order)
// Sets related.R.User.
func (o *User) RemoveBoards(ctx context.Context, exec boil.ContextExecutor, related ...*Board) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.Boards {
			...
			ln := len(o.R.Boards)
			if ln > 1 && i < ln-1 {
				o.R.Boards[i] = o.R.Boards[ln-1]
			}
			o.R.Boards = o.R.Boards[:ln-1]
			break
		}
	}
